const Auth=(()=>{
  ...
  function saveSession(data){if(!data?.token||!data?.expiresAt)throw new Error("Data sesi tidak valid.");const user=data.user?{UserID:data.user.UserID,Name:data.user.Name,SchoolID:data.user.SchoolID,Role:data.user.Role,Status:data.user.Status}:null;try{sessionStorage.setItem(KEY,JSON.stringify({token:data.token,expiresAt:data.expiresAt,user}))}catch{throw new Error("Browser tidak dapat menyimpan sesi.")}}
...
document.querySelector("#login-form")?.addEventListener("submit",async event=>{event.preventDefault();const form=event.currentTarget,button=form.querySelector("button[type=submit]"),message=form.querySelector(".form-message");let completed=false;Utils.setButtonLoading(button,true,"Masuk…");message.textContent="";try{const result=await apiPost("login",Utils.formObject(form));Auth.saveSession(result.data);completed=true;form.reset();Utils.toast("Berhasil masuk.","success");location.replace(Utils.pageUrl(Auth.homeForRole(result.data.user?.Role)))}catch(error){message.textContent=Utils.errorMessage(error);Utils.toast(message.textContent,"error")}finally{if(!completed)Utils.setButtonLoading(button,false)}});

document.querySelector('[data-password-toggle]')?.addEventListener('click',event=>{const button=event.currentTarget,input=document.querySelector(`#${button.getAttribute('aria-controls')}`),show=input.type==='password';input.type=show?'text':'password';button.setAttribute('aria-pressed',String(show));button.setAttribute('aria-label',show?'Sembunyikan password':'Tampilkan password');button.querySelector('.eye-open').hidden=show;button.querySelector('.eye-closed').hidden=!show;input.focus()});
